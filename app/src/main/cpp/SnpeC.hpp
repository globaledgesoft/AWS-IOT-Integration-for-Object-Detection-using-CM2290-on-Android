...
class SnpeC {
    ...
    public:
        SnpeC(std::string &dlc, int system_type, std::vector<std::string> &output_layers);
        SnpeC(const SnpeC &qc);
		std::map<std::string, std::vector<float>> obj_predict(cv::Mat input_image);
